...
import { ref } from 'vue'
import { getArticle, deleteArticle, changeArticleState } from "@/api/article";
...
const initGetUsersList = async () => {
  const res = await getArticle(queryForm.value)
...
const changeState = async (info) => {
  await changeArticleState(info.id, info.state)
  ElMessage({
    message: '状态更新成功',
    type: 'success',
  })
